...
const DEFAULT_PRODUCTS = [
  { id: 'p1', name: 'Classic Oxford Shirt', category: 'Shirts', price: 1299, originalPrice: 1599, stock: 24, sizes: ['S','M','L','XL','XXL'], image: 'https://images.unsplash.com/photo-1596755094514-f87e34085b2c?w=400&h=500&fit=crop', description: 'Premium cotton oxford shirt with button-down collar. Perfect for formal and smart-casual looks.', badge: 'Bestseller' },
  { id: 'p2', name: 'Slim Fit Chino Trousers', category: 'Trousers', price: 1499, originalPrice: 1899, stock: 18, sizes: ['28','30','32','34','36'], image: 'https://images.unsplash.com/photo-1624378439575-d8705ad7ae80?w=400&h=500&fit=crop', description: 'Comfortable stretch chino trousers in versatile beige. Ideal for everyday wear.', badge: null },
  { id: 'p3', name: 'Linen Summer Shirt', category: 'Shirts', price: 999, originalPrice: 1299, stock: 8, sizes: ['S','M','L','XL'], image: 'https://images.unsplash.com/photo-1602810318383-e386cc2a3ccf?w=400&h=500&fit=crop', description: 'Breathable pure linen shirt for Delhi summers. Light and stylish.', badge: 'Low Stock' },
  { id: 'p4', name: 'Premium Denim Jacket', category: 'Jackets', price: 2499, originalPrice: 2999, stock: 12, sizes: ['M','L','XL','XXL'], image: 'https://images.unsplash.com/photo-1551028719-00167b16eac5?w=400&h=500&fit=crop', description: 'Classic denim jacket with modern wash. Layer it over any outfit.', badge: 'New' },
  { id: 'p5', name: 'Formal Black Trousers', category: 'Trousers', price: 1699, originalPrice: null, stock: 30, sizes: ['28','30','32','34','36','38'], image: 'https://images.unsplash.com/photo-1594938298603-c8148c4dae35?w=400&h=500&fit=crop', description: 'Sharp formal trousers for office and events. Wrinkle-resistant fabric.', badge: null },
  { id: 'p6', name: 'Casual Polo T-Shirt', category: 'T-Shirts', price: 799, originalPrice: 999, stock: 45, sizes: ['S','M','L','XL','XXL'], image: 'https://images.unsplash.com/photo-1586790170083-2f9ceadc732d?w=400&h=500&fit=crop', description: 'Soft pique cotton polo. Available in multiple colors.', badge: null },
  { id: 'p7', name: 'Wool Blend Blazer', category: 'Jackets', price: 3999, originalPrice: 4599, stock: 6, sizes: ['M','L','XL'], image: 'https://images.unsplash.com/photo-1507679799987-c73779587ccf?w=400&h=500&fit=crop', description: 'Sophisticated wool-blend blazer for special occasions and winter.', badge: 'Exclusive' },
  { id: 'p8', name: 'Cargo Utility Pants', category: 'Trousers', price: 1799, originalPrice: 2199, stock: 15, sizes: ['30','32','34','36'], image: 'https://images.unsplash.com/photo-1473966968600-fa801b869a1a?w=400&h=500&fit=crop', description: 'Functional cargo pants with multiple pockets. Street-style ready.', badge: null },
  { id: 'p9', name: 'Mandarin Collar Shirt', category: 'Shirts', price: 1199, originalPrice: 1499, stock: 20, sizes: ['S','M','L','XL'], image: 'https://images.unsplash.com/photo-1607345366928-199ea26cfe3e?w=400&h=500&fit=crop', description: 'Modern mandarin collar shirt in soft cotton. Indo-western vibes.', badge: 'Popular' },
  { id: 'p10', name: 'Graphic Print Tee', category: 'T-Shirts', price: 599, originalPrice: 799, stock: 0, sizes: ['S','M','L','XL'], image: 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=400&h=500&fit=crop', description: 'Bold graphic print t-shirt. Limited edition design.', badge: 'Sold Out' },
  { id: 'p11', name: 'Leather Belt Classic', category: 'Accessories', price: 899, originalPrice: null, stock: 40, sizes: ['32','34','36','38','40'], image: 'https://images.unsplash.com/photo-1624222247344-550fb60583fd?w=400&h=500&fit=crop', description: 'Genuine leather belt with matte buckle. Everyday essential.', badge: null },
  { id: 'p12', name: 'Winter Quilted Jacket', category: 'Jackets', price: 2799, originalPrice: 3299, stock: 10, sizes: ['M','L','XL','XXL'], image: 'https://images.unsplash.com/photo-1544923246-77307dd62870?w=400&h=500&fit=crop', description: 'Warm quilted jacket for Delhi winters. Lightweight yet cozy.', badge: 'New' }
];

const ADMIN = { email: 'admin@uniqueman.com', password: 'admin123', name: 'Admin', role: 'admin' };
...
function toggleWishlist(productId) {
  ...
  if (!session) { showToast('Please login to use wishlist', 'error'); return false; }
  const all = getFromStorage(STORAGE_KEYS.wishlist, {});
  let list = all[session.id] || [];
  const exists = list.includes(productId);
  if (exists) { list = list.filter(id => id !== productId); showToast('Removed from wishlist'); }
  else { list.push(productId); showToast('Added to wishlist', 'success'); }
...
function getStockText(stock) { if (stock <= 0) return 'Out of Stock'; if (stock <= 10) return 'Only ' + stock + ' left'; return 'In Stock'; }
  ...
  return '<div class="product-card" data-id="' + product.id + '">' +
    ...
    '<div class="product-info">' +
      ...
      '<div class="product-actions">' +
        '<button class="btn btn-primary btn-sm" style="flex:1" onclick="openPrebookModal(\'' + product.id + '\')">Pre-Book</button>' +
...
function openPrebookModal(productId) {
  ...
  if (!isLoggedIn()) { showToast('Please login to pre-book items', 'error'); setTimeout(() => window.location.href = 'login.html', 1200); return; }
  ...
  overlay.innerHTML = '<div class="modal"><div class="modal-header"><h3>Pre-Book: ' + product.name + '</h3><button class="modal-close" onclick="closeModal(\'prebook-modal\')">&times;</button></div><div class="modal-body"><div class="flex gap-2 mb-2" style="gap:1rem"><img src="' + product.image + '" alt="" style="width:80px;height:100px;object-fit:cover;border-radius:8px" onerror="this.src=\'https://via.placeholder.com/80x100\'"><div><strong>' + product.name + '</strong><div class="product-price mt-1">' + formatPrice(product.price) + '</div><div class="product-stock ' + getStockClass(product.stock) + '">' + getStockText(product.stock) + '</div></div></div><form id="prebook-form"><input type="hidden" name="productId" value="' + product.id + '"><div class="form-group"><label>Select Size *</label><div class="size-options" id="size-options">' + sizesHtml + '</div><input type="hidden" name="size" id="selected-size" required></div><div class="form-row"><div class="form-group"><label>Quantity *</label><input type="number" name="quantity" class="form-control" min="1" max="5" value="1" required></div><div class="form-group"><label>Preferred Date</label><input type="date" name="preferredDate" class="form-control" min="' + new Date().toISOString().split('T')[0] + '"></div></div><div class="form-group"><label>Your Phone *</label><input type="tel" name="phone" class="form-control" placeholder="9876543210" required pattern="[0-9]{10}"></div><div class="form-group"><label>Notes (optional)</label><textarea name="notes" class="form-control" rows="2" placeholder="Any special request..."></textarea></div></form></div><div class="modal-footer"><button class="btn btn-secondary" onclick="closeModal(\'prebook-modal\')">Cancel</button><button class="btn btn-primary" onclick="submitPrebook()">Confirm Pre-Booking</button></div></div>';
...
function submitPrebook() {
  ...
  closeModal('prebook-modal'); showToast('Pre-booking confirmed! We will contact you soon.', 'success');
}
...
function openProductDetail(productId) {
  ...
  overlay.innerHTML = '<div class="modal" style="max-width:640px"><div class="modal-header"><h3>' + product.name + '</h3><button class="modal-close" onclick="closeModal(\'detail-modal\')">&times;</button></div><div class="modal-body"><div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem"><img src="' + product.image + '" alt="' + product.name + '" style="width:100%;border-radius:12px;aspect-ratio:3/4;object-fit:cover" onerror="this.src=\'https://via.placeholder.com/300x400\'"><div><div class="product-category">' + product.category + '</div><div class="product-price" style="font-size:1.4rem;margin:0.5rem 0">' + formatPrice(product.price) + (product.originalPrice ? '<span class="original">' + formatPrice(product.originalPrice) + '</span>' : '') + '</div><div class="product-stock ' + getStockClass(product.stock) + ' mb-2">' + getStockText(product.stock) + '</div><p style="color:var(--coffee-700);margin-bottom:1rem;font-size:0.95rem">' + product.description + '</p><p style="font-size:0.85rem;color:var(--coffee-600)"><strong>Available sizes:</strong> ' + product.sizes.join(', ') + '</p><div class="mt-2" style="display:flex;gap:0.5rem;flex-wrap:wrap"><button class="btn btn-primary" onclick="closeModal(\'detail-modal\');openPrebookModal(\'' + product.id + '\')">Pre-Book Now</button><button class="btn btn-outline" onclick="handleWishlist(\'' + product.id + '\');closeModal(\'detail-modal\')">' + (isInWishlist(product.id) ? 'Remove from Wishlist' : 'Add to Wishlist') + '</button></div></div></div></div></div>';
  overlay.classList.add('active');
}
...
function renderHeader(activePage = '') {
  ...
  return '<header class="header"><div class="nav-container">' +
    '<a href="index.html" class="logo">Uniqueman</a>' +
...
function renderFooter() {
  return '<footer class="footer"><div class="footer-grid">' +
    '<div><h4>Uniqueman Men\'s Wear</h4><p>#EXCLUSIVE Men\'s Clothing Outlet</p><p>B-210, 1st Floor, Jhilmil<br>Near Garam Masala, Delhi 110095</p></div>' +
    '<div><h4>Quick Links</h4><a href="index.html">Home</a><a href="products.html">Shop All</a><a href="wishlist.html">Wishlist</a><a href="contact.html">Contact Us</a></div>' +
    '<div><h4>Contact</h4><p>📞 +91 87962 46647</p><a href="https://wa.me/918796246647" target="_blank">WhatsApp Us</a><a href="https://www.instagram.com/_uniqueman__/" target="_blank">Instagram @_uniqueman__</a></div>' +
    '<div><h4>Hours</h4><p>Mon – Sat: 11:00 AM – 9:00 PM</p><p>Sunday: 12:00 PM – 8:00 PM</p></div>' +
    '</div><div class="footer-bottom">© ' + new Date().getFullYear() + ' Uniqueman Men\'s Wear. All rights reserved. | Built with ☕</div></footer>';
}

document.addEventListener('DOMContentLoaded', () => { initData(); updateWishlistBadge(); });
